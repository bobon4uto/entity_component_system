...
u meta_macro_count_args_for_each(String_Holder in,
  ...
  for (u i = 0; i < in.count; ++i) {
    //  printf("comparing: (%*s == %s)", (i32)target_function_len, in.items + i,
    //  target_function);
...
String_Holder
make_meta_macro_function_infinitely_variadic(u count_needed,
                                             const char *function_name) {
  String_Holder sh0 = {0};
  String_Holder *sh = &sh0;
  sh_appendf(sh,
             "// DISCLAIMER: this file is generated by " __FILE__ ":"
             "%d"
             "\n",
             __LINE__);
  ...
  sh_appendf(sh, "#define %s(...) _%s_COUNT(__VA_ARGS__", function_name,
             function_name);
...
String_Holder inf(String_Holder *in, char *function_name) {
  ...
  printf("count = %zu\n", count_needed);
  String_Holder out0 =
      make_meta_macro_function_infinitely_variadic(count_needed, function_name);
  return out0;
}
String_Holder meta_macro_get_type(String_Holder *in, u position) {
  ...
  for (; i < in->count; ++i) {
    c = in->items[i];
    if (c == '/' && precomment && !multiline_comment) {
      comment = true;
      precomment = false;
    }
    if (c == '/' && precomment && multiline_comment) {
    ...
    if (c == ';' && !is_struct_body && !comment && !macro)
      break;
  }
  u typename_len = 0;
  --i;
...
int main(int argc, char **argv) {
  ...
  bool do_print = false;
  if (argc < 2) {
    printf("usage: %s [-m mode{inf|type|just}] [-o output] [-p]{print to "
           "stdout} [-f function_name] file\n"
           "inf = infinite variadic macro (allow function(...) with as many "
           "args as you want)\n"
           "just = print what program read\n"
           "type = create macro that will act as array of ALL typedef'ed "
  ...
  switch (mode) {
  case 'i':
    out0 = inf(in, function_name);
    break;
  case 't':
    out0 = types(in);
    break;
  case 'j':
    sh_append_buf(&out0, in->items, in->count);
    printf("%s", sh_c_str(in));
    break;
  }
  String_Holder *out = &out0;
